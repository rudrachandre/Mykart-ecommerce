...
interface OverviewData {
  ...
  kpis: {
    revenue: KPIItem;
    orders: KPIItem;
    productsSold: KPIItem;
    uniqueCustomers: KPIItem;
    avgOrderValue: KPIItem;
    refunds: KPIItem;
  };
  trends: Array<{ date: string; revenue: number; orders: number; cancelledOrders: number }>;
...
export default function AdminAnalyticsPage() {
  ...
  const maxOrders = Math.max(...trends.map((t) => t.orders + t.cancelledOrders), 1);
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 pb-6 border-b">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Real-time revenue, order metrics, and category breakdown derived strictly from database records.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        ...
        <div className="bg-card p-6 rounded-2xl border shadow-sm relative overflow-hidden">
          <div className="flex justify-between items-start mb-4">
            <div>
              <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Net Sales Revenue</p>
              <h2 className="text-2xl sm:text-3xl font-extrabold text-foreground mt-1">
                {formatCurrency(kpis?.revenue.value || 0)}
              </h2>
            </div>
          ...
          <div className="flex items-center justify-between pt-3 border-t text-xs">
            <span className="text-muted-foreground">
              Prev: <span className="font-medium text-foreground">{formatCurrency(kpis?.revenue.prevValue || 0)}</span>
            </span>
            {renderBadge(kpis?.revenue.change || 0, kpis?.revenue.text || '0%')}
          </div>
        </div>
        ...
        <div className="bg-card p-6 rounded-2xl border shadow-sm relative overflow-hidden">
          ...
          <div className="flex items-center justify-between pt-3 border-t text-xs">
            ...
            {renderBadge(kpis?.orders.change || 0, kpis?.orders.text || '0%')}
          </div>
        </div>
        ...
        <div className="bg-card p-6 rounded-2xl border shadow-sm relative overflow-hidden">
          <div className="flex justify-between items-start mb-4">
            <div>
              <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Products Sold</p>
              <h2 className="text-2xl sm:text-3xl font-extrabold text-foreground mt-1">
                {kpis?.productsSold.value || 0}
              </h2>
            </div>
          ...
          <div className="flex items-center justify-between pt-3 border-t text-xs">
            <span className="text-muted-foreground">
              Prev: <span className="font-medium text-foreground">{kpis?.productsSold.prevValue || 0} units</span>
            </span>
            {renderBadge(kpis?.productsSold.change || 0, kpis?.productsSold.text || '0%')}
          </div>
        </div>
        ...
        <div className="bg-card p-6 rounded-2xl border shadow-sm relative overflow-hidden">
          ...
          <div className="flex items-center justify-between pt-3 border-t text-xs">
            ...
            {renderBadge(kpis?.uniqueCustomers.change || 0, kpis?.uniqueCustomers.text || '0%')}
          </div>
        </div>
        ...
        <div className="bg-card p-6 rounded-2xl border shadow-sm relative overflow-hidden">
          ...
          <div className="flex items-center justify-between pt-3 border-t text-xs">
            ...
            {renderBadge(kpis?.avgOrderValue.change || 0, kpis?.avgOrderValue.text || '0%')}
          </div>
        </div>

        {/* KPI 6: Refunds & Returns */}
        <div className="bg-card p-6 rounded-2xl border shadow-sm relative overflow-hidden">
          <div className="flex justify-between items-start mb-4">
            <div>
              <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Refunds & Returns</p>
              <h2 className="text-2xl sm:text-3xl font-extrabold text-foreground mt-1">
                {formatCurrency(kpis?.refunds.value || 0)}
              </h2>
            </div>
          ...
          <div className="flex items-center justify-between pt-3 border-t text-xs">
            <span className="text-muted-foreground">
              {kpis?.refunds.count || 0} claims processed
            </span>
            {renderBadge(kpis?.refunds.change || 0, kpis?.refunds.text || '0%')}
          </div>
        </div>
      </div>
          ...
          <div className="flex justify-between items-center mb-6">
            <div>
              <h3 className="text-base font-bold text-foreground flex items-center gap-2">
                <TrendingUp className="w-5 h-5 text-emerald-600" /> Daily Revenue Trend
              </h3>
              <p className="text-xs text-muted-foreground mt-0.5">Historical price snapshot gross sales per day</p>
            </div>
            <span className="text-xs font-semibold px-2.5 py-1 bg-emerald-500/10 text-emerald-600 rounded-full border border-emerald-500/20">
              Total: {formatCurrency(kpis?.revenue.value || 0)}
            </span>
          </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block bg-popover text-popover-foreground text-xs p-2.5 rounded-lg shadow-xl border whitespace-nowrap z-30">
                        ...
                        <p className="text-emerald-600 font-bold mt-0.5">{formatCurrency(item.revenue)}</p>
        ...
        <div className="bg-card p-6 rounded-2xl border shadow-sm">
          <div className="flex justify-between items-center mb-6">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">Completed vs Cancelled order volume per day</p>
            </div>
            ...
            <div>
              <div className="flex items-end gap-1 h-64 border-b border-l pb-2 pl-2 pt-4 relative">
                {trends.map((item, idx) => {
                  ...
                  const pct = (totalCount / maxOrders) * 100;
                  return (
                    <div
                      key={idx}
                    ...
                    >
                      ...
                      <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block bg-popover text-popover-foreground text-xs p-2.5 rounded-lg shadow-xl border whitespace-nowrap z-30">
                        <p className="font-bold text-foreground">{item.date}</p>
                        <p className="text-blue-600 font-bold mt-0.5">{item.orders} Completed</p>
        ...
        <div className="lg:col-span-2 bg-card p-6 rounded-2xl border shadow-sm">
          <div className="flex justify-between items-center mb-6">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">Sales distribution across parent product categories</p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-card p-6 rounded-2xl border shadow-sm">
          <div className="flex justify-between items-center mb-6">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">Ranked by historical item snapshot revenue</p>
            </div>
          </div>
            ...
            {topProducts.map((prod, idx) => (
              <div key={prod.id || idx} className="py-3.5 first:pt-0 flex items-center justify-between text-xs">
                ...
                <div className="text-right">
                  ...
                  <p className="text-[10px] text-muted-foreground">{prod.quantity} sold</p>
                </div>
              </div>
            ))}
        ...
        <div className="bg-card p-6 rounded-2xl border shadow-sm">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h3 className="text-base font-bold text-foreground flex items-center gap-2">
                <Store className="w-5 h-5 text-indigo-600" /> Top Brands by Revenue
              </h3>
              <p className="text-xs text-muted-foreground mt-0.5">Brand performance across all processed items</p>
            </div>
          </div>
